...
namespace CC.Runtime.Utils
{
    public static class StringUtils
    {
		...
		public static string ToStr(this Prop[] props)
		{
			string str = "{";
			string gap = "";
			foreach(Prop prop in props)
			{
				str += gap + prop.ToString();
				gap = ", ";
			}
		...
		public static string ToStrProp(this float[] props)
		{
			string str = "{";
			string gap = "";
			for(int i = 0; i < props.Length; i ++)
			{
				if(props[i] != 0)
				{
					...
					str += gap + Prop.CreateInstance(i, props[i]).ToString();
					gap = ", ";
				}
			}
        ...
        public static T GetObject<T>(this string[] src, int idx) where T : class,IParseField,new()
        {
            if( idx < src.Length && !string.IsNullOrEmpty(src[idx]) ){
                T res = new T();
                res.ParseField(src[idx]);
                return res;
            }
            else{
                return null;
