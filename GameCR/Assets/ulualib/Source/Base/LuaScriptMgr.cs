...
public class LuaScriptMgr
{
	...
	LuaFunction applicationPause = null;
	LuaFunction applicationFocus = null;
...
//#endif
		...
		applicationPause = GetLuaFunction("OnApplicationPause");
		applicationFocus = GetLuaFunction("OnApplicationFocus");
	...
	{
		applicationPause.Call(isPause);
	}

	public void OnApplicationFocus(bool focusStatus)
	{
		applicationFocus.Call(focusStatus);
	}
    ...
    public byte[] Loader(string name)
    {
		if(GameConst.DevelopMode == false)
		{
			if(luaFileAB != null)
			{
				string assetName = Util.LuaAssetBundleAssetName(name);
				TextAsset textAsset = (TextAsset)luaFileAB.LoadAsset(assetName);
				if(textAsset != null)
				{
				...
				{
					UnityEngine.Debug.Log(string.Format("<color=red>[LuaScriptMgr.Loader] name={0},  assetName={1}</color>", name, assetName));
				}
			}
		}
		...
		{
			UnityEngine.Debug.Log(string.Format("<color=red>[LuaScriptMgr.Loader] name={0},  path={1}</color>", name, path));
		}
#else
